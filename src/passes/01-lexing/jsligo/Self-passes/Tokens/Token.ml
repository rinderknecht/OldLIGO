...
module T =
 ...
 (* | Lang     of lexeme Region.reg *)
    ...
    | New      of lexeme Wrap.t  (* new      *)
    | Of       of lexeme Wrap.t  (* of       *)
    | Return   of lexeme Wrap.t  (* return   *)
    | Static   of lexeme Wrap.t  (* static   *)
...
(*  | Void     t *)
 ...
 (* | With     t *)
    ...
    (* JavaScript Keywords *)
     ...
     let wrap_new      = wrap "new"
     let wrap_of       = wrap "of"
     let wrap_return   = wrap "return"
     let wrap_static   = wrap "static"
  ...
  (* let wrap_with     = wrap "with"    *)
     ...
     let mk_New      region = New      (wrap_new      region)
     let mk_Of       region = Of       (wrap_of       region)
     let mk_Return   region = Return   (wrap_return   region)
     let mk_Static   region = Static   (wrap_static   region)
  ...
  (*   mk_With;   *)
    ...
    let ghost_new      = wrap_new      Region.ghost
    let ghost_of       = wrap_of       Region.ghost
    let ghost_return   = wrap_return   Region.ghost
    let ghost_static   = wrap_static   Region.ghost
 ...
 (* let ghost_with     = wrap_with     Region.ghost *)
    ...
    let ghost_New      = New     ghost_new
    let ghost_Of       = Of      ghost_of
    let ghost_Return   = Return  ghost_return
    let ghost_Static   = Static  ghost_static
...
(*  let ghost_Lang     l = Lang     (ghost_lang l) *)
 ...
 (* | "Lang"     -> "[%Michelson" *)
    ...
    | "New"      -> ghost_new#payload
    | "Of"       -> ghost_of#payload
    | "Static"   -> ghost_static#payload
 ...
 (* | Lang {value = {value = payload; _}; region; _} ->
    ...
    | New      t -> t#region, sprintf "New%s" (comments t)
    | Of       t -> t#region, sprintf "Of%s" (comments t)
    | Return   t -> t#region, sprintf "Return%s" (comments t)
    | Static   t -> t#region, sprintf "Static%s" (comments t)
