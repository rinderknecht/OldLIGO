...
module Make (Options : Options.S) (Token : Token.S) =
  struct
    ...
    let mk_tez nat suffix state buffer =
      let state, Region.{region; _} = state#sync buffer
      and tez = (Z.of_string nat) in
      try
        let tez_64 = Z.to_int64 tez in
        if   Int64.equal tez_64 Int64.zero && String.(nat <> "0")
        then fail region Non_canonical_zero
        else (Token.mk_tez nat ~suffix tez_64 region, state)
      with Z.Overflow -> fail region Overflow_tez
      ...
      if Z.equal (Q.den q_mutez) Z.one then
        try
          ...
          then fail region Non_canonical_zero
               ...
               (Token.mk_mutez lexeme ~suffix mutez_64 region, state)
