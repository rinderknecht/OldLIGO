...
and print_I_Const state (node : intf_const reg) =
  ...
  let thread =
    match kwd_const with
      None -> print_variable const_name
    | Some kwd_const ->
        token kwd_const ^^ space ^^ print_variable const_name in
  let thread = match const_optional with
    | None -> thread
    | Some qmark -> thread ^^ token qmark
...
and print_T_ParameterOf state (node : parameter_of_type reg) =
  ...
  let print = print_namespace_selection state
  in token kwd_parameter_of ^^ print_chevrons state print namespace_path
