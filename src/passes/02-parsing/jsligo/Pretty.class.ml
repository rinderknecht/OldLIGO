...
let unroll_C_Attr (attr, class_member) =
  let rec aux attrs = function
    C_Attr (attr, member) -> aux (attr :: attrs) member
  | member                -> List.rev attrs, member
  in aux [attr] class_member
...
and print_D_Class state (node : class_decl reg) =
  let {kwd_class; class_name; implements; class_body} = node.value in
  let thread = token kwd_class ^^ space ^^ token class_name in
  let thread = thread ^^ space ^^ print_implements state implements
  in group (thread ^^ print_class_body state class_body)

and print_class_body state (node : class_members braces) =
  print_braces ~force_hardline:true state (print_class_members state) node

and print_class_members state (node : class_members) =
  print_nsep_or_term (break 1) (print_class_member state) node
...
and print_C_Attr state (node : attribute * class_member) =
  let attributes, member = unroll_C_Attr node in
  let thread = print_class_member state member
  in print_attributes state thread attributes

and print_C_Field state (node : field_member reg) =
  let {pattern; field_type; eq; field_init} = node.value in
  failwith "print_C_Field"

and print_implements state (node : interface option) =
...
and print_interface state (node : interface) =
  ...
  let intf_exprs = print_nsepseq (break 1) (print_intf_expr state) intf_exprs
  in token kwd_implements ^^ space ^^ intf_exprs
