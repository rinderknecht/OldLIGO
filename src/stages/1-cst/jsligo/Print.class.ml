...
and print_declaration state = function
  D_Class     d -> print_D_Class     state d
...
and print_D_Class state (node: class_decl reg) =
  let Region.{value; region} = node in
  let {kwd_class=_; class_name; implements; class_body} = value in
  let children = Tree.[
    mk_child     print_class_name    class_name;
    mk_child_opt print_interface     implements;
    mk_child     print_class_members class_body]
  in Tree.make ~region state "D_Class" children

and print_class_name state (node: class_name) =
  Tree.(make_unary state "<class>" make_literal node)

and print_class_members state (node: class_members braces) =
  let Region.{value; region} = node in
  let members = value.inside in
  let children = Tree.(mk_children_nsep_or_term print_class_member members)
  in Tree.make state ~region "<members>" children

and print_class_member state = function
  C_Attr   m -> print_C_Attr   state m
| C_Field  m -> print_C_Field  state m
| C_Method m -> print_C_Method state m

and print_C_Attr state (node: attribute * class_member) =
  let attribute, member = node in
  let children = Tree.[
    mk_child print_attribute    attribute;
    mk_child print_class_member member]
  in Tree.make state "C_Attr" children

and print_C_Field state (node: field_member reg) =
  let Region.{region; value} = node in
  let {pattern; field_type; eq=_; field_init} = value in
  let children = Tree.[
    mk_child     print_binders         pattern;
    mk_child_opt print_type_annotation field_type;
    mk_child     print_field_init      field_init]
  in Tree.make state ~region "C_Field" children

and print_field_init state (node: expr) =
  Tree.make_unary state "<init>" print_expr node

and print_C_Method state (node: method_member reg) =
  let Region.{region; value} = node in
  let {method_name; generics; parameters; rhs_type; method_body} = value in
  let children = Tree.[
    mk_child     print_variable      method_name;
    mk_child_opt print_generics      generics;
    mk_child     print_fun_params    parameters;
    mk_child_opt print_rhs_type      rhs_type;
    mk_child     print_decl_fun_body method_body]
  in Tree.make ~region state "C_Method" children
...
and print_T_Sum state (node : sum_type) =
  ...
  Tree.of_nsep_or_pref ~region state "T_Sum"
                       (print_variant_kind ~root:"Variant" print_type_expr) value

and print_variant_kind
  : 'a. root:string -> 'a Tree.printer -> 'a variant_kind Tree.printer =
 fun ~root printer state node ->
  match node with
    Variant node -> print_variant ~root printer state node
  | Legacy  node -> print_legacy_variant printer state node

and print_variant
  : 'a. root:string -> 'a Tree.printer -> 'a variant reg Tree.printer =
 fun ~root printer state node ->
  ...
  let children =
    ...
    @ mk_children_attr attributes
  in Tree.make ~region:node.region state root children
...
and print_P_CtorApp state (node: pattern variant_kind) =
  print_variant_kind ~root:"P_CtorApp" print_pattern state node
...
and print_E_CtorApp state (node : expr variant_kind) =
  print_variant_kind ~root:"E_CtorApp" print_expr state node
