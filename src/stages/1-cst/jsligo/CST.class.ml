...
type class_name     = lexeme wrap [@@deriving yojson_of] (* C *)
...
and class_decl = {
  kwd_class  : (kwd_class [@yojson.opaque]);
  class_name : class_name;
  implements : interface option;
  class_body : class_members braces
}

and class_members = (class_member, (semi [@yojson.opaque])) nsep_or_term

and class_member =
  C_Attr   of ((attribute [@yojson.opaque]) * class_member)
| C_Field  of field_member reg
| C_Method of method_member reg
