...
type _ sing =
  ...
  | S_class_decl : class_decl sing
  | S_class_name : class_name sing
  | S_class_members : class_members sing
  | S_class_member : class_member sing
  | S_field_member : field_member sing
  | S_method_member : method_member sing
  | S_method_name : fun_name sing
...
let fold'
  ...
  | S_class_name -> process @@ node -| S_wrap S_lexeme
  | S_field_member ->
    let {pattern; field_type; eq; field_init} = node in
    process_list
    [ pattern -| S_pattern
    ; field_type -| S_option S_type_annotation
    ; eq -| S_equal
    ; field_init -| S_expr ]
  | S_class_member -> process
    ( match node with
      C_Attr node -> node -| S_array_2 (S_attribute, S_class_member)
    | C_Field node -> node -| S_reg S_field_member
    | C_Method node -> node -| S_reg S_method_member
    )
  | S_method_member ->
    let {method_name; generics; parameters; rhs_type; method_body} = node in
    process_list
    [ method_name -| S_method_name
    ; generics -| S_option S_generics
    ; parameters -| S_fun_params
    ; rhs_type -| S_option (S_array_2 (S_colon, S_type_expr))
    ; method_body -| S_braces S_statements ]
  | S_cases -> process
    ( match node with
  ...
  | S_declaration -> process
    ( match node with
        D_Class node -> node -| S_reg S_class_decl
  ...
  | S_class_decl ->
    let {kwd_class; class_name; implements; class_body} = node in
    process_list
      [ kwd_class -| S_kwd_class
      ; class_name -| S_class_name
      ; implements -| S_option S_interface
      ; class_body -| S_braces S_class_members ]
  | S_class_members ->
    process @@ node -| S_nsep_or_term (S_class_member, S_comma)
  | S_method_name -> process @@ node -| S_variable
  | S_kwd_class -> process @@ node -| S_wrap S_lexeme
  | S_fun_params ->
